"""
================================================================================
ETAPA 4 - Análise dos resultados e visualizações
================================================================================

O enunciado pede:
   • Validação cruzada                                 → Gráfico 2
   • Gráficos com a base antes e depois                → Gráficos 3 e 5
   • Conhecimentos descobertos                         → gráfico de importância
   • Percentuais de acerto/erro com diferentes amostras → Gráfico 1

Esta etapa NÃO treina nada novo (com uma exceção: a curva de aprendizado do
Gráfico 1 precisa retreinar a árvore em fatias progressivas do dataset).
Tudo o que vem de step3_models é apenas consumido aqui.

Os seis paineis montam uma narrativa completa:

   1. Curva de aprendizado
      Mostra acurácia de treino e teste à medida que aumentamos a fatia de
      dados usada para treinar. É o gráfico canônico de "memorização vs
      generalização": treino sempre alto, teste sobe e estabiliza - quanto
      menor o gap, melhor o modelo está generalizando.

   2. Validação Cruzada k=10 (fold a fold)
      Cada barra é um dos 10 folds. Verde = acima da média, vermelho = abaixo.
      Pouca variação entre folds = estimativa confiável.

   3. Matriz de Confusão
      Mostra ONDE o modelo erra. Em geral confunde Meia ↔ Atacante (papéis
      próximos) e quase nunca confunde Goleiro com qualquer outra posição
      (porque os atributos 'goalkeeping_*' são quase exclusivos deles).

   4. Elbow Method
      Inércia (azul) decresce conforme K aumenta - sempre. O que importa é o
      "cotovelo": onde a queda começa a achatar. A silhueta (laranja) ajuda
      a confirmar: K bom = silhueta alta sem cair muito.

   5. Clusters em 2D via PCA
      Os 12 atributos viraram 2 componentes principais para conseguirmos
      plotar. Se os pontos com a mesma cor formam ilhas, o K-Means achou
      estrutura real.

   6. Comparativo Final
      Barra a barra: Árvore vs K-Means nas duas formas de avaliação.

GRÁFICO BÔNUS - Importância dos atributos da árvore.
   Esse é o gráfico que responde "o que o modelo aprendeu". Os atributos
   verdes (top 5) são os que carregaram mais Ganho de Informação durante
   a construção da árvore - efetivamente, é o que ele considerou MAIS
   discriminativo entre as 4 classes.
================================================================================
"""
...
def step4_compare(data, models, output_dir=BASE_DIR, show=True):
    ...
    eixos[0, 0].plot(percentuais_treino, acuracias_no_treino, 'o-',
                     color='steelblue', label='Treino', linewidth=2)
    eixos[0, 0].plot(percentuais_treino, acuracias_no_teste, 's-',
                     color='tomato', label='Teste', linewidth=2)
    ...
    eixos[0, 0].yaxis.set_major_formatter(
    ...
    cores_fold = [
        'seagreen' if s >= acuracia_cv_media else 'tomato' for s in scores_cv
    ]
    eixos[0, 1].bar(range(1, 11), scores_cv, color=cores_fold, edgecolor='white')
    eixos[0, 1].axhline(acuracia_cv_media, color='black', linestyle='--',
                        linewidth=1.5, label=f'Média: {acuracia_cv_media:.1%}')
    eixos[0, 1].set_title('Validação Cruzada k=10 (Árvore)')
    eixos[0, 1].set_xlabel('Fold')
    eixos[0, 1].set_ylabel('Acurácia')
    eixos[0, 1].yaxis.set_major_formatter(
        plt.FuncFormatter(lambda v, _: f'{v:.0%}')
    )
    eixos[0, 1].legend()
    ...
    sns.heatmap(
        matriz_confusao, annot=True, fmt='d', ax=eixos[0, 2], cmap='Blues',
    ...
    eixo_esq.plot(valores_k, lista_inercias, 'o-',
                  color='steelblue', label='Inércia')
    eixo_dir.plot(valores_k, lista_silhouettes, 's--',
                  color='darkorange', label='Silhouette')
    eixo_esq.axvline(numero_de_posicoes, color='gray', linestyle=':',
    ...
    eixos[1, 1].scatter(
        X_em_2d[:, 0], X_em_2d[:, 1],
        c=clusters_encontrados, cmap='tab10', alpha=0.5, s=10,
    )
    eixos[1, 1].set_title(
    ...
    eixos[1, 2].bar(posicoes_barras - largura / 2, valores_arvore,
                    largura, label='Árvore de Decisão',
                    color='steelblue', alpha=0.85)
    eixos[1, 2].bar(posicoes_barras + largura / 2, valores_kmeans,
                    largura, label='K-Means',
                    color='darkorange', alpha=0.85)
    for i, (va, vk) in enumerate(zip(valores_arvore, valores_kmeans)):
        eixos[1, 2].text(i - largura / 2, va + 0.005,
                         f'{va:.1%}', ha='center', fontweight='bold')
        eixos[1, 2].text(i + largura / 2, vk + 0.005,
                         f'{vk:.1%}', ha='center', fontweight='bold')
    ...
    eixos[1, 2].yaxis.set_major_formatter(
        plt.FuncFormatter(lambda v, _: f'{v:.0%}')
    )
    eixos[1, 2].legend()
    ...
    importancia_ord.plot(
        kind='barh', ax=eixo_imp, color=cores_imp,
